<template>
    <div class="asset">
        <div class="icon" :djtx="isDjtxToken">
            <img v-if="iconUrl" :src="iconUrl" />
            <p v-else>?</p>
        </div>
        <p class="name_col not_mobile">
            {{ name }} ({{ symbol }})
            <span v-if="!isDjtxToken">ANT</span>
        </p>
        ...
        <p class="balance_col" v-if="isBalance">
            ...
            <br />
            <span class="fiat" v-if="isDjtxToken">
...
export default class FungibleRow extends Vue {
    ...
    get iconUrl(): string | null {
        ...
        if (this.isDjtxToken) {
            return '/img/djtx_icon_circle.png'
        }
